        ...
        viewModelScope.launch {
            try {
                ...
                if (user != null) {
                    ...
                    if (user.profile_pic != null) CurrentSession.setUserData(user.username, user.email, user.profile_pic, user.language, user.rank_quiz, user.rank_event)
                    else CurrentSession.setUserData(user.username, user.email, "", user.language, user.rank_quiz, user.rank_event)
                }
            }
            catch (e: Exception){
                val code = when (e) {
